...
Important: the installer does not force-reset an existing dirty `~/.codex/superpowers` checkout. It skips repo refresh in that case so local edits are not discarded.
